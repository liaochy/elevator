...
public class SyslogServerHandler extends SimpleChannelUpstreamHandler {
    private final static Log logger = LogFactory.getLog(SyslogServerHandler.class);
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e)
        ...
        parse();
        MessageHandler.handleMessage(this.ident, this.message, logHandler);
    }
    ...
    protected void parseIdent() {
        ...
        if (i > -1) {
            ...
            this.message = this.message.substring(i + 2);
            parseProcessId();
        }else
        {
            throw new IllegalArgumentException("SyslogServerHandler.parseIdent:the argument ident is blank!");
    ...
    protected void parse() {
        logger.info("SyslogServerHandler.parse info:" + this.message);
